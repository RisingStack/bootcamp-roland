...
async function insertOrReplace({repository, user, line_count}){
    ...
    const test = ['bela', 'janos', 'gyuri'];
    console.log(...test);
    const response = await db.raw(`
        INSERT INTO contribution ("user",repository,line_count) 
        VALUES (${user}, ${repository}, ${line_count})
        ON CONFLICT ("user") DO UPDATE SET "user"=EXCLUDED."user", repository=EXCLUDED.repository, line_count=EXCLUDED.line_count
    `);
    return response;
}

/*async function read({user: {userId, login}, repository: {repositoryId, full_name}}){
    const tables = [];
    if(user){}
}*/
